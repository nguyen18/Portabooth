...
| Save/Share/Print handlers | `saveBtn` → `canvas.toDataURL('image/png')` download link. `shareBtn` → `navigator.share` with a `File` (only shown if `navigator.canShare` exists). `printBtn` → builds two `<img>` copies of the strip (from `stripCanvas.toDataURL()`) into `#printSheet`, then `window.print()` — see "Print layout" below. `againBtn` → hides the result screen to shoot again. |
