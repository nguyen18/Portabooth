...
| Save/Share/Print handlers | `saveBtn` → `canvas.toDataURL('image/png')` download link. `shareBtn` → `navigator.share` with a `File` (only shown if `navigator.canShare` exists). `printBtn` → async; builds two `<img>` copies of the strip (from `stripCanvas.toDataURL()`) into `#printSheet`, `await`s both images' `decode()` (or `load`/`error` as a fallback), then `window.print()` — see "Print layout" below for why the wait matters. `againBtn` → hides the result screen to shoot again. |
...
`#filterSelect` (`default` / `bw` / `sepia` / `saturated`, mapped via `PHOTO_FILTERS` to CSS filter strings `'none'` / `'grayscale(100%)'` / `'sepia(100%)'` / `'saturate(220%)'`) applies to the 4 captured photos only, not the caption/date text or cut-guide lines. Applied via `ctx.filter` on the `CanvasRenderingContext2D` right before each shot's `drawImage()` call in `buildStrip()`, reset to `'none'` immediately after — this is a canvas *rendering* property, not a CSS style on the `<canvas>` element, which matters: a CSS filter on the element would only affect on-screen display, while `ctx.filter` actually bakes the effect into the canvas's pixel data, so it correctly shows up in `toDataURL()`/`toBlob()` output (Save/Share/Print).
...
`drawCaptionLines()`'s `captionYOffset` (`layout.dateLine ? 4 : 6`) looks like it should be simplifiable — e.g. "just use the same offset either way," or "use 0 when there's no date, since that's the last line and needs the full `CAPTION_PAD` below it." **Don't** — both were tried and measured wrong. The caption (32px) and date (22px) lines are different font sizes with different descent metrics, so the same offset does *not* produce the same visual bottom margin when the caption is the last line (no date) vs. when the date is. `4` and `6` were reached by empirically sweeping offset values and measuring actual ink-to-bottom-edge distance on the rendered canvas (scanning `getImageData` from the bottom up) until the no-date margin matched the with-date margin exactly (48px each, at current constants) — they are not derived from a formula. If `CAPTION_FONT_SIZE`, `DATE_FONT_SIZE`, or the Unna font itself ever changes, re-sweep and re-measure rather than assuming a "cleaner" value is correct.

Note this offset is a small (4-6px) fine-tuning detail, now overshadowed by the much larger, deliberate padding difference described in `computeCaptionLayout()` above (captionH staying constant regardless of date) — the two are independent and both still apply.
