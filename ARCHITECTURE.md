...
`#filterSelect` (`default` / `bw` / `sepia`, mapped via `PHOTO_FILTERS` to CSS filter strings `'none'` / `'grayscale(100%)'` / `'sepia(100%)'`) applies to the 4 captured photos only, not the caption/date text or cut-guide lines. Applied via `ctx.filter` on the `CanvasRenderingContext2D` right before each shot's `drawImage()` call in `buildStrip()`, reset to `'none'` immediately after — this is a canvas *rendering* property, not a CSS style on the `<canvas>` element, which matters: a CSS filter on the element would only affect on-screen display, while `ctx.filter` actually bakes the effect into the canvas's pixel data, so it correctly shows up in `toDataURL()`/`toBlob()` output (Save/Share/Print).
